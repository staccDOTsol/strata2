...
export class SplTokenBonding extends AnchorSdk<any> {
  ...
  async buyInstructions({
    ...
    if (baseAmount) {
      ...
      if (expectedOutputAmount) {
        ...
        min = await curve.buyWithBaseAmount(
          maxPrice,
          tokenBondingAcct.buyBaseRoyaltyPercentage,
          tokenBondingAcct.buyTargetRoyaltyPercentage,
          unixTime
  ...
  async swap({
    ...
    baseMint,
    targetMint,
    baseAmount,  isBuy = true, // Add this line with a default value
    ...
    const two = !targetMint.equals(new PublicKey("C7yuH1a4XHfCSBG6c6MZQuCSuKrHsCBsxHxfpMNUQ9Vs"))
    ? new PublicKey("C7yuH1a4XHfCSBG6c6MZQuCSuKrHsCBsxHxfpMNUQ9Vs") :  new PublicKey("BQpGv6LVWG1JRm1NdjerNSFdChMdAULJr3x9t2Swpump")
    const one = targetMint.equals(new PublicKey("C7yuH1a4XHfCSBG6c6MZQuCSuKrHsCBsxHxfpMNUQ9Vs"))
     ? new PublicKey("C7yuH1a4XHfCSBG6c6MZQuCSuKrHsCBsxHxfpMNUQ9Vs") :  new PublicKey("BQpGv6LVWG1JRm1NdjerNSFdChMdAULJr3x9t2Swpump")
    const hierarchyFromTarget = await this.getBondingHierarchy(
      (
        await SplTokenBonding.tokenBondingKey(one)
      )[0],
      two
    );
    console.log("Starting swap function");
    console.log("Payer:", payer.toBase58());
    console.log("Source Authority:", sourceAuthority.toBase58());
    console.log("Base Mint:", baseMint.toBase58());
    console.log("Target Mint:", targetMint.toBase58());
    console.log("Base Amount:", baseAmount);
    console.log("Expected Base Amount:", expectedBaseAmount);
    console.log("Desired Target Amount:", desiredTargetAmount);
    console.log("Expected Output Amount:", expectedOutputAmount);
    console.log("Slippage:", slippage);
    console.log("Balance Check Tries:", balanceCheckTries);
    console.log("Entangled:", entangled);

    const hierarchyFromBase = await this.getBondingHierarchy(
      (
        await SplTokenBonding.tokenBondingKey(two)
      )[0],
      one
    );

    console.log("Hierarchy from Base:", hierarchyFromBase);
    ...
    for (const [index, subHierarchy] of hierarchyToTraverse.entries()) {
      ...
      if (isBuy) {
        ...
        ({ instructions, signers } = await this.sellInstructions({
          ...
          tokenBonding: tokenBonding.publicKey,
          expectedOutputAmount: true ? expectedOutputAmount : undefined,
          slippage,
        }));
  ...
  async sellInstructions({
   ...
   // targetAmount *= ba
    const args: IdlTypes<any>["SellV0Args"] = {
      targetAmount: toBN(targetAmount, baseMint),
      minimumPrice: new BN(
    ...
    if (isNative) {
      ...
      const bestPool = await this.findBestPool(baseMint);
